...
#define INKSTAND_SCENE_DEFAULT_FRAME_MS 33U
#define INKSTAND_SCENE_DEFAULT_SETTLE_FRAMES 40U
#define INKSTAND_SCENE_DEFAULT_START_MS 1000U

/* GIF carries a delay in centiseconds in a 16-bit field, so 655350 ms is the ceiling anything
   downstream can express. */
#define INKSTAND_SCENE_DELAY_CEILING_MS 600000U

/* A typo guard, not a claim about what is sensible: a missed digit in a hold is ten minutes. */
#define INKSTAND_SCENE_NUMBER_MAX 600000L
...
int inkstand_scene_init(struct inkstand_scene *scene, const struct inkstand_scene_host *host,
    ...
    if (host == NULL || host->capture == NULL || host->snapshot == NULL || host->drain == NULL ||
        host->press == NULL) {
        return inkstand_scene_fail(scene, "the host needs a capture, a snapshot, drain and press");
    }
    for (size_t i = 0; i < host->verb_count; ++i) {
        if (host->verbs[i].name == NULL || host->verbs[i].run == NULL) {
            return inkstand_scene_fail(scene, "verb %zu has no name or no run", i);
        }
        for (size_t j = 0; j < sizeof scene_generic_verbs / sizeof scene_generic_verbs[0]; ++j) {
            if (strcmp(host->verbs[i].name, scene_generic_verbs[j]) == 0) {
                return inkstand_scene_fail(scene, "'%s' is the runner's own verb",
                                           host->verbs[i].name);
            }
        }
        for (size_t j = 0; j < i; ++j) {
            if (strcmp(host->verbs[i].name, host->verbs[j].name) == 0) {
                return inkstand_scene_fail(scene, "two verbs are called '%s'", host->verbs[i].name);
            }
        }
    }
    scene->host = *host;
    ...
    if (scene->config.frame_ms == 0U) {
        scene->config.frame_ms = INKSTAND_SCENE_DEFAULT_FRAME_MS;
    }
    if (scene->config.settle_frames == 0U) {
        scene->config.settle_frames = INKSTAND_SCENE_DEFAULT_SETTLE_FRAMES;
    }
    if (scene->config.start_ms == 0U) {
        scene->config.start_ms = INKSTAND_SCENE_DEFAULT_START_MS;
    }
...
int inkstand_scene_number(struct inkstand_scene *scene, const char *text, const char *what,
    ...
    if (end == text || *end != '\0' || text[0] == '-' ||
        value > (unsigned long)INKSTAND_SCENE_NUMBER_MAX) {
        return inkstand_scene_fail(scene, "%s: '%s' is not a number I can use", what, text);
    }
    *out = (unsigned)value;
    return 0;
}
...
int inkstand_scene_signed(struct inkstand_scene *scene, const char *text, const char *what,
    ...
    if (end == text || *end != '\0' || value < -INKSTAND_SCENE_NUMBER_MAX ||
        value > INKSTAND_SCENE_NUMBER_MAX) {
        return inkstand_scene_fail(scene, "%s: '%s' is not a number I can use", what, text);
    }
    *out = (int)value;
    return 0;
 ...
 */
static void scene_advance(struct inkstand_scene *scene, unsigned ms) {
    scene->now_ms += ms;
...
static int scene_flush_delay(struct inkstand_scene *scene) {
    ...
    if (status < 0) {
        return inkstand_scene_fail(scene, "cannot record frame %u's delay: %s", scene->frames,
                                   strerror(-status));
    }
    return 0;
}

static int scene_emit_delay(struct inkstand_scene *scene, unsigned delay_ms) {
    struct inkstand_scene_host *host = &scene->host;
    ...
    if (scene->sink.frame != NULL) {
        ...
        if (status < 0) {
            return inkstand_scene_fail(scene, "cannot write frame %u: %s", scene->frames + 1U,
                                       strerror(-status));
        }
    }
    scene->frames++;
...
int inkstand_scene_settle(struct inkstand_scene *scene) {
    for (unsigned i = 0U; i < scene->config.settle_frames; ++i) {
        ...
        const int status = inkstand_scene_emit(scene);
        if (status < 0) {
            return status;
        }
    }
...
int inkstand_scene_press(struct inkstand_scene *scene, enum inkcell_key key) {
    ...
    const int status = inkstand_scene_emit(scene);
    return status < 0 ? status : inkstand_scene_settle(scene);
}
...
static int scene_hold(struct inkstand_scene *scene, unsigned extra_ms) {
    if (scene->frames == 0U) {
        return inkstand_scene_fail(scene, "hold: nothing has been captured yet");
    }
    const unsigned held = scene->pending_delay_ms + extra_ms;
    scene->pending_delay_ms =
        held > INKSTAND_SCENE_DELAY_CEILING_MS ? INKSTAND_SCENE_DELAY_CEILING_MS : held;
 ...
 */
static int scene_apply_theme(struct inkstand_scene *scene, const char *name) {
    ...
    if (theme == NULL) {
        ...
        return inkstand_scene_fail(scene, "no theme called '%s'. Try:%s", name, known);
    }
...
static int scene_start(struct inkstand_scene *scene) {
    if (scene->started) {
        return 0;
    }
    scene->started = true;
    ...
    if (scene->host.seed_count > 0U) {
        const struct inkstand_scene_seed *seed = NULL;
        ...
        if (seed == NULL) {
            return inkstand_scene_fail(scene, "scene: no seed called '%s'", scene->seed);
        }
    ...
    return inkstand_scene_emit(scene);
}

/* ---- the generic verbs --------------------------------------------------------------------- */

static int scene_setup_value(struct inkstand_scene *scene, const char *command, char **rest,
                             char **out) {
    if (scene->started) {
        return inkstand_scene_fail(scene, "'%s' has to come before the first frame", command);
    }
    *out = inkstand_scene_word(rest);
    if (*out == NULL) {
        return inkstand_scene_fail(scene, "'%s' needs a value", command);
    }
    return 0;
}
 ...
 */
static int scene_clock(struct inkstand_scene *scene, char *rest) {
    if (scene->started) {
        return inkstand_scene_fail(scene, "'clock' has to come before the first frame");
    }
    const char *when = inkstand_scene_tail(rest);
    int year = 0;
    int month = 0;
    int day = 0;
    int hour = 0;
    ...
    if (sscanf(when, "%4d-%2d-%2d %2d:%2d", &year, &month, &day, &hour, &minute) != 5) {
        return inkstand_scene_fail(scene, "'clock' needs a local time as YYYY-MM-DD HH:MM");
    }
    struct tm parts;
    ...
    if (pinned <= 0) {
        return inkstand_scene_fail(scene, "'clock' cannot represent that time here");
    }
...
static int scene_run_generic(struct inkstand_scene *scene, const char *command, char *rest) {
    char *value = NULL;
    int status = 0;
    ...
    if (strcmp(command, "scale") == 0) {
        unsigned scale = 0U;
        if ((status = scene_setup_value(scene, command, &rest, &value)) < 0 ||
            (status = inkstand_scene_number(scene, value, "scale", &scale)) < 0) {
            return status;
        }
    ...
    if (strcmp(command, "delay") == 0) {
        ...
        return inkstand_scene_number(scene, value, "delay", &scene->config.delay_ms);
    }
    ...
    if (strcmp(command, "theme") == 0) {
        value = inkstand_scene_word(&rest);
        if (value == NULL) {
            return inkstand_scene_fail(scene, "'theme' needs a name");
        }
        if (!scene->started) {
    ...
    if (strcmp(command, "pointer") == 0) {
        ...
        return scene->started ? inkstand_scene_emit(scene) : 0;
    }
    ...
    if (strcmp(command, "key") == 0) {
        ...
        if (name == NULL) {
            return inkstand_scene_fail(scene, "'key' needs a button");
        }
        const enum inkcell_key key = inkcell_key_from_name(name);
        if (key == INKCELL_KEY_NONE) {
            return inkstand_scene_fail(scene, "no button called '%s'", name);
        }
        unsigned count = 1U;
        if (count_text != NULL &&
            (status = inkstand_scene_number(scene, count_text, "key count", &count)) < 0) {
            return status;
        }
        ...
        for (unsigned i = 0U; i < count && status == 0; ++i) {
            status = inkstand_scene_press(scene, key);
        }
        return status;
    }

    if (strcmp(command, "frame") == 0) {
        if ((status = scene_start(scene)) < 0 || (status = inkstand_scene_emit(scene)) < 0) {
            return status;
        }
        ...
        return inkstand_scene_settle(scene);
    }
    ...
    if (strcmp(command, "hold") == 0) {
        ...
        unsigned ms = 0U;
        if (value == NULL) {
            return inkstand_scene_fail(scene, "'hold' needs a duration in ms");
        }
        if ((status = inkstand_scene_number(scene, value, "hold", &ms)) < 0 ||
...
int inkstand_scene_run_line(struct inkstand_scene *scene, char *line) {
    ...
    scene->error[0] = '\0';
    char *rest = line;
    const char *command = inkstand_scene_word(&rest);
    ...
    const struct inkstand_scene_verb *verb = NULL;
    ...
    if (verb == NULL) {
        return inkstand_scene_fail(scene, "unknown command '%s'", command);
    }

    int status = 0;
    if ((verb->flags & INKSTAND_SCENE_SETUP) != 0U) {
        if (scene->started) {
            return inkstand_scene_fail(scene, "'%s' has to come before the first frame", command);
        }
    ...
    if ((status = verb->run(scene, rest, scene->host.userdata)) < 0) {
        if (scene->error[0] == '\0') {
            (void)inkstand_scene_fail(scene, "'%s' failed: %s", command, strerror(-status));
        }
        return status;
    }
    if ((verb->flags & (INKSTAND_SCENE_SETUP | INKSTAND_SCENE_NO_FRAME)) != 0U) {
        return 0;
    }
    if ((status = inkstand_scene_emit(scene)) < 0) {
        return status;
    }
    return inkstand_scene_settle(scene);
}

#define INKSTAND_SCENE_LINE_MAX 512U

int inkstand_scene_run_file(struct inkstand_scene *scene, FILE *file) {
    char line[INKSTAND_SCENE_LINE_MAX];
    while (fgets(line, (int)sizeof line, file) != NULL) {
        line[strcspn(line, "\r\n")] = '\0';
        const int status = inkstand_scene_run_line(scene, line);
        if (status < 0) {
            return status;
        }
    }
