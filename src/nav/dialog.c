/*
 * A two-answer question over the screen - see inkstand/nav/dialog.h.
 */
...
void inkstand_dialog_open(struct inkstand_dialog *dialog, uint16_t subject) {
    ...
    dialog->open = true;
    dialog->cursor = INKSTAND_DIALOG_CANCEL; /* so a repeated press changes nothing */
...
enum inkstand_dialog_press inkstand_dialog_key(struct inkstand_dialog *dialog,
    ...
    if (dialog == NULL || !dialog->open) {
        return INKSTAND_DIALOG_IGNORED;
    }
    switch (key) {
    case INKCELL_KEY_UP:
    ...
    case INKCELL_KEY_RIGHT: {
        const uint8_t to = inkstand_dialog_answer(map, focus_base, key, dialog->cursor);
        if (to == dialog->cursor) {
            return INKSTAND_DIALOG_IGNORED;
        }
        dialog->cursor = to;
        return INKSTAND_DIALOG_MOVED;
    }
    case INKCELL_KEY_A:
    case INKCELL_KEY_START: {
        const bool accepted = dialog->cursor == INKSTAND_DIALOG_ACCEPT;
        if (out_subject != NULL) {
            *out_subject = dialog->subject;
        }
        inkstand_dialog_close(dialog);
        return accepted ? INKSTAND_DIALOG_ACCEPTED : INKSTAND_DIALOG_CANCELLED;
    }
    case INKCELL_KEY_B:
        if (out_subject != NULL) {
            *out_subject = dialog->subject;
        ...
        inkstand_dialog_close(dialog);
        return INKSTAND_DIALOG_CANCELLED;
    default:
        return INKSTAND_DIALOG_IGNORED;
    }
}
