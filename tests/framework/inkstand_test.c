...
#define INKSTAND_TEST_SUITE_NAME_MAX 128

static struct inkstand_test_case *g_cases = NULL;
 ...
 */
static const char *suite_of(const struct inkstand_test_case *test, char *out, size_t cap) {
...
static int compare_cases(const void *lhs, const void *rhs) {
    const struct inkstand_test_case *const *left = lhs;
    const struct inkstand_test_case *const *right = rhs;

    char left_suite[INKSTAND_TEST_SUITE_NAME_MAX];
    char right_suite[INKSTAND_TEST_SUITE_NAME_MAX];
...
static struct inkstand_test_case **ordered_cases(size_t *count) {
    *count = g_case_count;
    if (g_case_count == 0U) {
        return NULL;
    }

    struct inkstand_test_case **ordered = calloc(g_case_count, sizeof(*ordered));
    if (ordered == NULL) {
    ...
    size_t index = 0U;
    for (struct inkstand_test_case *node = g_cases; node != NULL; node = node->next) {
        ordered[index++] = node;
    }
...
static bool test_selected(const struct inkstand_test_case *test,
...
static void select_tests(const struct test_options *options, size_t *selected, bool *ran_any) {
    ...
    for (size_t i = 0; i < count; ++i) {
        const struct inkstand_test_case *test = ordered[i];
...
static void print_usage(void) {
    printf("Usage: inkstand_tests [options]\n");
    printf("Options:\n");
